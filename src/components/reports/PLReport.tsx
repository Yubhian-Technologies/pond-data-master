        ...
        if (snap.exists()) {
          ...
          const normalized: PLData = {
            testCode: Array.from(
              ...
              (_, i) => savedPlData.testCode?.[i] || `${i + 1}`,
            ),
            rostralSpines: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.rostralSpines?.[i] || "-",
            ),
            avgLength: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.avgLength?.[i] || "-",
            ),
            plAge: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.plAge?.[i] || "-",
            ),
            sizeVariation: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.sizeVariation?.[i] || "-",
            ),
            mgr: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.mgr?.[i] || "-",
            ),
            mgrPercent: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.mgrPercent?.[i] || "-",
            ),
            hp: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.hp?.[i] || "-",
            ),
            shg: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.shg?.[i] || "-",
            ),
            necAppend: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.necAppend?.[i] || "-",
            ),
            necRostrum: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.necRostrum?.[i] || "-",
            ),
            necGill: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.necGill?.[i] || "-",
            ),
            necMuscle: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.necMuscle?.[i] || "-",
            ),
            foulAppend: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.foulAppend?.[i] || "-",
            ),
            foulGill: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.foulGill?.[i] || "-",
            ),
            foulAbdomen: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.foulAbdomen?.[i] || "-",
            ),
            stressTankSalinity: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.stressTankSalinity?.[i] || "-",
            ),
            salinityPercent: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.salinityPercent?.[i] || "-",
            ),
            totalScore: Array.from(
              { length: allSampleCount },
              (_, i) => savedPlData.totalScore?.[i] || "-",
            ),
          };
    ...
    <div className="rounded-lg p-8 bg-white" id="pl-report">
      ...
      <div className="w-full mb-12">
        <table className="w-full border-2 border-gray-800 text-sm table-fixed">
          <tbody>
            <tr>
              <td className="font-semibold bg-blue-100 border px-2 py-1">
                Farmer Name
              </td>
              ...
              <td className="border px-4 py-2">
                {formatDateDDMMYYYY(farmerInfo.sampleDate)}
              </td>
            </tr>
            <tr>
              <td className="font-semibold bg-blue-100 border px-4 py-2">
      ...
      <div className="flex justify-center overflow-x-auto">
        <table className="inline-table border-4 border-black text-xs whitespace-nowrap">
          <thead>
            <tr className="bg-blue-100">
              <th className="border-2 border-black px-2 py-1 font-semibold">
                TEST CODE
              </th>
          ...
          <tbody>
            {plRows.map((row) => (
              <React.Fragment key={row.key}>
                <tr className={row.key === "totalScore" ? "bg-blue-100" : ""}>
                  <td
                    className={`border-2 border-black px-2 py-1 font-semibold ${
                      row.key === "totalScore" ? "bg-blue-100" : "bg-gray-50"
                    }`}
                  >
                    {row.label}
                  </td>
                  {plData[row.key].map((val, i) => (
                    <td
                      ...
                      className={`border-2 border-black px-4 py-3 text-center font-medium text-xs ${
        ...
        <>
          <div className="mt-20 border-t-2 border-black pt-8">
            ...
            <div className="text-center text-xs text-gray-700">
              <p>
                <strong>Note:</strong> The samples brought by Farmer, the
                Results Reported above are meant for Guidance only for
          ...
          <div className="text-sm text-gray-800 mt-8">
            ...
            <p className="mt-6">
              ...
              brought by Farmer, the Results Reported above are meant for
              guidance only for Aquaculture Purpose. Not
            </p>
          </div>
